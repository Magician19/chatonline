//WebScoket初始化
window.CHAT = {
    serverAddr: "ws://" + window.location.hostname + ":8081/ws",
    socket: null,
    username: '',
    ...
    init: function (username,roomId) {
        ...
        if (window.WebSocket) {
            ...
            CHAT.socket.onopen = function () {
                ...
                CHAT.send("[LOGIN][" + new Date().getTime() + "][" + username + "][" + roomId +"]");
            };
            ...
            layui.use('layer', function () {
                var layer = layui.layer;
                layer.msg("你的浏览器不支持WebSocket协议，请换谷歌或火狐试试！！");
            });
    ...
    //发送鲜花
    sendFlower: function (roomId) {
        if (!window.WebSocket) {
            return;
        }
        if (CHAT.socket.readyState == WebSocket.OPEN) {
            var message = ("[FLOWER][" + new Date().getTime() + "]" + "[" + CHAT.username + "][" + roomId + "]");
            CHAT.send(message);
            $("#send-Message").focus();
        } else {
            layui.use('layer', function () {
